...
| Path | Role |
|------|------|
| `App.tsx` | UI root: `GestureHandlerRootView`, preferences, i18n sync, bottom sheets, safe area, navigation. |
| `navigation/` | `NavigationContainer`, root stack (`Main` → tabs, `Profile`), floating tabs Map / List / Settings. |
...
| `preferences/` | User preferences context and persistence (AsyncStorage). |
...
| `@react-native-async-storage/async-storage` | Local persistence (preferences). |
...
- **`app.json` → plugins:** `expo-dev-client`, `react-native-edge-to-edge`, `expo-localization`, `expo-location` (when-in-use message). **Google Maps** keys in `ios.config.googleMapsApiKey` and `android.config.googleMaps.apiKey` (rotate if the repo is public).
